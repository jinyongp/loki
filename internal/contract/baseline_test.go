...
func TestCurrentContract(t *testing.T) {
	...
	if len(definitions) != 35 {
		t.Fatalf("current public definitions = %d, want 35", len(definitions))
	}
	for _, required := range []string{"system_inspect", "browser_session", "workspace_edit", "secret_write", "github_read", "github_write", "github", "github_issue_fields_read", "github_issue_fields_write", "project_coordination", "project_coordination_write", "agent_guidance", "job"} {
		if !seen[required] {
