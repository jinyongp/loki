...
func TestDestinationPolicy(t *testing.T) {
	...
	for _, value := range []string{"http://example.test", "https://example.test/path?q=1", "http://127.0.0.1:43000"} {
		if _, err := ValidateURL(value); err != nil {
			t.Fatal(value, err)
		}
	}
	for _, value := range []string{"file:///etc/passwd", "http://user:pass@example.test", "http://localhost:43000", "http://127.0.0.1:8765", "http://10.0.0.1", "http://[::1]", "http://example.test:8000", "http://169.254.169.254", "http://[::ffff:127.0.0.1]"} {
		if _, err := ValidateURL(value); err == nil {
			t.Fatal(value)
		}
	}
	p := Policy{ValidatePort: func(context.Context, int) bool { return true }, Lookup: func(context.Context, string) ([]netip.Addr, error) {
