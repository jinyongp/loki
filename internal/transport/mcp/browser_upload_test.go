...
func TestBrowserUploadHandlerStagesPathsBeforeRPC(t *testing.T) {
	...
	handler := BrowserHandlers(client, nil, nil, nil)["browser_interact"]
...
func TestBrowserRPCStagesPolicyCheckedWorkspaceFiles(t *testing.T) {
	...
	stager := SocketBrowserUploadStager{Socket: socket}

	stage, err := stager.StageBrowserUpload(t.Context(), files, []string{"one.txt", "nested/one.txt"})
	if err != nil {
		t.Fatal(err)
	}
	defer stage.Cleanup()
	...
	for _, path := range []string{".env", "link.txt"} {
		if _, err := stager.StageBrowserUpload(t.Context(), files, []string{path}); err == nil {
