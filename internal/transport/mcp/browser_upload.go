...
type SocketBrowserUploadStager struct {
	Socket string
}
...
func (c SocketBrowserUploadStager) StageBrowserUpload(ctx context.Context, files *workspace.Files, paths []string) (_ *stagedBrowserUpload, err error) {
	...
	inbox := filepath.Join(filepath.Dir(c.Socket), "uploads")
