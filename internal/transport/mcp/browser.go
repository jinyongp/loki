...
func BrowserHandlers(client BrowserCaller, uploads BrowserUploadStager, files *workspace.Files, store *artifacts.Store) map[string]mcpserver.Handler {
	...
	for tool, actions := range map[string]map[string]string{
		...
		handlers[tool] = func(ctx context.Context, args map[string]any) (*mcp.CallToolResult, error) {
			...
			if tool == "browser_interact" && action == "upload" {
				...
				stager := uploads
				if stager == nil {
					stager, _ = client.(BrowserUploadStager)
				}
				if stager == nil {
