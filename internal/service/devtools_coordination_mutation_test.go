...
func (r *recordingCoordinationMutator) MutateCoordination(_ context.Context, cwd string, action devtools.CoordinationMutation, request devtools.CoordinationMutationRequest) (devtools.CoordinationMutationResult, error) {
...
func TestDevtoolsCoordinationMutationRuntimeOperationIsFixedAndTyped(t *testing.T) {
	mutator := &recordingCoordinationMutator{result: devtools.CoordinationMutationResult{
		...
		Context: sessionClaimContext, ContextValid: true, RunID: sessionRunID, TaskID: sessionTaskID,
	}}
	...
	if mutator.action != devtools.CoordinationClaim || mutator.cwd != "repo" || mutator.request.Target != sessionTaskID {
		t.Fatalf("runtime mutation = %#v", mutator)
	}
	row := result.(map[string]any)
	if row["context"] != sessionClaimContext || row["run_id"] != sessionRunID || row["task_id"] != sessionTaskID {
