...
func TestAssembledMCPHTTPAndShutdown(t *testing.T) {
	...
	app, err := NewMCP(c, MCPOptions{Runtime: runtime, PortGuard: guard, Browser: browser, Jobs: jobControllerFixture(), GitJobs: gitJobsFixture(t, c, nil), Ports: ports, Policy: generation, Token: token, Access: accessFixture("mcp-access"), PreviewAccess: accessFixture("preview-access"), Environment: map[string]string{"GIT_CONFIG_GLOBAL": "/dev/null", "GIT_CONFIG_NOSYSTEM": "1"}})
	if err != nil {
		t.Fatal(err)
	}
	defer app.Close()
...
func TestNewMCPAgentGuidanceUsesNativeProvider(t *testing.T) {
	...
	app, err := NewMCP(c, MCPOptions{
		...
		GitJobs: gitJobsFixture(t, c, nil), Ports: ports,
