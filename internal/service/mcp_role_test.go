...
func TestMCPRoleReadinessAndCancellation(t *testing.T) {
	...
	go func() {
		done <- RunMCP(ctx, c, MCPOptions{Runtime: runtime, PortGuard: runtime, Browser: browser, Ports: ports, Policy: policyGenerationFixture(t), Token: strings.Repeat("t", 43)}, listener, func() error { close(ready); return nil })
	}()
	select {
	case <-ready:
	case err := <-done:
