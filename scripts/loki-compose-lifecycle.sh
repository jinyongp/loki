...
unpack(){ local n=$1 dir=$2 i v; i=$(cat "$state/current-image"); v=$(vol "$n"); dc volume create "$v">/dev/null; dc run --rm --network none --cap-drop ALL --cap-add DAC_OVERRIDE --cap-add FOWNER --security-opt no-new-privileges --entrypoint /bin/sh --mount "type=volume,src=$v,dst=/target" --mount "type=bind,src=$dir,dst=/backup,readonly" "$i" -ec 'find /target -mindepth 1 -maxdepth 1 -exec rm -rf -- {} +&&tar -xf "/backup/$1.tar" -C /target' sh "$n"; }
