...
`go run ./tools/readinessprobe` reproduces the remaining synthetic observations recorded in `docs/go-readiness-review.md` for R1, R3, R7, R11, and R14. R4 is covered by permanent managed-credential regressions in `internal/secret`, `internal/service`, and `internal/devtools`; R5 is covered by permanent repository-execution regressions in `internal/gitops`; R8/R9 are covered by permanent regressions in `internal/toolchain/install_test.go` and `internal/toolchain/archive_safety_test.go`.
