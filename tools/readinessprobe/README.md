...
`go run ./tools/readinessprobe` reproduces the remaining synthetic observations recorded in `docs/go-readiness-review.md` for R1, R3, R4, R5, R7, R11, and R14. R8/R9 are covered by permanent regressions in `internal/toolchain/install_test.go` and `internal/toolchain/archive_safety_test.go` instead.
