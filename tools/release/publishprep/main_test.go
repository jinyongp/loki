...
func writePublicationCandidate(t *testing.T, commit string) string {
	...
	generation, err := releases.NewGeneration(releases.GenerationSpec{
		Version: "1.2.3", ReleasedAt: time.Date(2026, 9, 22, 12, 0, 0, 0, time.UTC),
		HostBinaryDigest: "sha256:" + hex.EncodeToString(hostSum[:]),
		CoreImageDigest:  "sha256:" + strings.Repeat("b", 64),
		Components:       []releases.Component{{Name: "browser", Digest: "sha256:" + strings.Repeat("c", 64), Optional: true}},
		...
		Rollback: releases.RollbackCoverage{StateSnapshot: true, ConfigSnapshot: true, OptionalComponentState: []string{"browser"}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Cleanup(func() { inspectPublicationBootstrap = previousInspect })
...
func TestPreparePublicationProducesReleaseAndPagesInputs(t *testing.T) {
	...
	if outputRaw, err := exec.Command("sh", "-n", filepath.Join(output, "pages", "install.sh")).CombinedOutput(); err != nil {
...
func TestPreparePublicationRejectsBootstrapReleaseBindingDrift(t *testing.T) {
	commit := strings.Repeat("a", 40)
	candidate := writePublicationCandidate(t, commit)
	previous := inspectPublicationBootstrap
	inspectPublicationBootstrap = func(context.Context, string) (bootstrapinfo.Info, error) {
		return bootstrapinfo.Info{ReleaseTag: "v9.9.9", ReleaseManifestSHA256: strings.Repeat("a", 64), HostBinarySHA256: strings.Repeat("b", 64)}, nil
	}
...
func TestRenderInstallerRequiresExactPlaceholders(t *testing.T) {
	...
	rendered, err := renderInstaller([]byte("tag=@@LOKI_RELEASE_TAG@@\nsum=@@LOKI_BOOTSTRAP_SHA256@@\n"), "v1.2.3", digest)
	if err != nil {
		t.Fatal(err)
	}
