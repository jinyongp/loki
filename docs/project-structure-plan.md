...
A03/S03 status: the first concrete slice now exists as `internal/work/jobs`, `internal/work/jobs/remote`, `internal/app/launcher`, `cmd/loki-launcher`, `internal/app/executor`, and `cmd/loki-executor`. The executor role is a required architecture-check root whose transitive production closure excludes launcher implementation, platform credential storage, MCP, browser, provider-private-key and devtools code. The launcher role remains separately privileged and is the only production role that reaches `platform/sandbox`. The executor-facing API is still one-shot, but the launcher boundary now owns a bounded asynchronous job lifecycle with `start`, `wait`, and `cancel`, trusted timeout, result retention/capacity, shutdown cleanup, and explicit cleanup cancellation when the remote adapter loses a caller. Job journals/local persistence, gateway remote client, MCP binding, bounded output, cgroup ownership/OOM handling and real OCI acceptance remain later S03/A05/A07 work.
