...
This document now serves as the active architecture and implementation record. A03-A07 source work, including the public Job/network/endpoint slice, is implemented and the accumulated deterministic closeout gate has passed; the remaining blocker for acceptance completion is the supported-host real OCI/network run described above. A08 source work is in progress: Git/workspace execution now routes through confined Jobs, move/index CAS and linked-worktree semantics have been tightened, the workspace owner lives under `internal/work/workspace`, and its MCP bindings live under `internal/transport/mcp/workspace`; the integrated A08 acceptance gate is still open. A09-A14, release enablement, operational cutover and deletion of retained legacy deployment remain subsequent work and must not be inferred from the completed earlier slices.
