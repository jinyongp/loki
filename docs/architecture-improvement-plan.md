...
The public contract must be sufficient for an agent to choose and invoke the correct operation without relying on hidden implementation knowledge. The [MCP tool contract review](mcp-tool-contract-review.md) is the catalog-level evidence and design record. The current Go contract has 33 tools and 20 action unions; the contract audit reports zero flat action unions, 203 described public input properties out of 203, zero missing or open output schemas, and zero missing MCP annotations. The public `job` surface and server/stack/job preview variants are part of that generated contract. Remaining contract work is therefore semantic and acceptance-oriented rather than a flat-schema census: common operation/correlation identity, complete stable error-envelope behavior, family-specific concurrency/recovery guarantees, model-facing tool-choice evidence and real execution/network acceptance. Treat those remaining gaps as contract defects, not documentation polish.
...
| A13 | Produce immutable release/index identities, notices/provenance and a release-bound thin bootstrap; replace first-install documentation | A09, A11, A12 | Fresh Ubuntu/WSL install from immutable GitHub Release artifacts with manifest-bound digest verification, no source checkout or development compiler required on host |
...
A03/A05/A06/A07 implementation status: the durable Job core, privileged launcher, unprivileged executor, remote adapters and sandbox remain separated by strict Unix-RPC boundaries. Public Job start requests now carry only workspace-relative CWD/argv, a reviewed logical network profile (`none` or `dependency-install`) and bounded logical endpoint declarations. Job IDs, effective-policy digests, OCI image/mount/security authority, Docker network identifiers, gateway credentials and host-port selection remain trusted-side concerns. The sandbox creates the exact workload/gateway/network resource domain, authenticates dependency-install egress with a per-Job credential, publishes requested gateway endpoints only on loopback ephemeral host ports and binds the resulting neutral endpoint leases to the exact Job incarnation in the durable journal. Recovery rejects replacement gateway/workload/network resources and changed host-port bindings; terminal cleanup releases leases so numeric port reuse cannot preserve authority. The MCP `job` contract exposes replay-safe start/inspect/output/cancel with logical network/endpoints and neutral lease metadata, while `preview_publish(action=job)` resolves and revalidates the exact endpoint lease instead of treating a port as service identity. Native and container packaging now wire MCP to the executor only, the executor to the launcher only, and Docker authority only to the launcher. Focused Job/preview/contract/packaging tests and the architecture checker pass for this source slice. The batch-closeout deterministic gate has also passed with `go test ./...`, `go test -race ./...`, `go vet ./...`, and `go build ./...`. The real supported-host OCI fixture now includes the A06 authenticated egress, endpoint, exact-domain recovery, Job preview HTTP/WebSocket and stale/reused-lease scenario, and its self-contained runner prepares the normal image/workspace/registry/allowlisted-authority inputs automatically. Execution remains pending on a trusted Linux Docker host with current Buildx/BuildKit. A03-A07 are therefore source-implemented with deterministic validation complete, but not acceptance-complete or release-enabled.

### Implementation and validation cadence

The dependency rows above define implementation order and authority prerequisites; they do not require a repository-wide validation run after every work unit. The default cadence for the remaining redesign is implementation-first with deferred integrated validation.

- Write or update regression tests, fixtures, contract snapshots, failure probes and acceptance scenarios together with the implementation that requires them. Leaving required validation code until the end of the entire redesign is not acceptable, because the intended invariant must remain reviewable beside the code that establishes it.
- During implementation, execute only checks whose result is needed to continue safely into dependent work. Prefer the narrowest useful package, compile, schema, contract or fixture check. If downstream implementation does not depend on the result, record the exact check and defer its execution.
- A work item may be recorded as implementation-complete while required validation remains pending. Keep the corresponding task open or explicitly annotated as validation-pending; do not convert implementation progress into a readiness claim.
- Do not repeatedly run `go test ./...`, broad `go test -race`, `go vet ./...`, `go build ./...`, architecture-wide checks or equivalent repository gates between ordinary work units. Accumulate those checks for the largest coherent implementation batch.
- Before release/distribution work consumes the completed redesign, run the deterministic integrated gate once over the accumulated implementation. If a gate fails, fix the root cause, run the smallest regression that proves the correction, then rerun the affected integrated checks. Run the complete required deterministic gate once more before closeout.
- Fixture-heavy Chromium, OCI, ACL, clean-host, credential-bound and recovery checks stay deferred to the relevant integration/release acceptance boundary unless their result is necessary to choose an implementation design or unblock dependent work. A required release check that is skipped remains a failed gate.
- Reviewable implementation commits may precede the integrated validation run. Such commits must not claim milestone or release readiness, and any known failed required check blocks further completion claims until corrected.
...
The A01-A06 structural slice now has its source implementation through the Job network/endpoint authority boundary, and A07's public MCP Job surface is wired on top of that boundary. The accumulated deterministic closeout gate has passed. The supported-host real OCI fixture is implemented for authenticated dependency egress, direct/bypass denial, exact gateway/network recovery, loopback endpoint publication, stale/reused host-port lease identity, and preview HTTP/WebSocket through Job-owned leases; its runner supplies the normal fixture inputs and still requires execution on a trusted disposable Linux Docker host with current Buildx/BuildKit. Until that required external fixture runs and passes, the execution capability remains deterministic implementation evidence rather than release-enabled or milestone-complete behavior.
...
Implementation changes should remain reviewable and commit-scoped to the work units above. During the implementation batch, record the exact deferred validation commands, prerequisites and remaining gaps; execution results are accumulated at the integrated validation boundary rather than regenerated after every commit. No implementation commit should claim readiness while required deterministic or integration gates remain pending or skipped. Update active plans when the new architecture replaces an old assumption; do not retain contradictory compatibility instructions as live guidance.

This document now serves as the active architecture and implementation record. A03-A07 source work, including the public Job/network/endpoint slice, is implemented and the accumulated deterministic closeout gate has passed; the remaining blocker for their acceptance completion is the supported-host real OCI/network run described above. A08 source and deterministic acceptance are complete: Git/workspace execution runs through confined Jobs, move/index CAS and linked-worktree semantics are covered, the workspace owner lives under `internal/work/workspace`, and its MCP bindings live under `internal/transport/mcp/workspace`; release readiness still inherits the shared supported-host OCI gate. A09 source and deterministic acceptance are also complete: immutable managed generations, exact/partial/floating selectors, Node and standalone pnpm providers, durable project-selected Job intent, read-only generation mounts, Loki-owned shims, concurrent/crash/multi-project fixtures, and `internal/work/toolchains` ownership all pass the full test/race/vet/build/archcheck gate. A10-A14, release enablement, operational cutover and deletion of retained legacy deployment remain subsequent work.
