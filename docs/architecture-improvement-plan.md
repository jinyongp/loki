...
Devtools becomes an ordinary utility/optional integration within this execution model, not the authority owner and not an assumed out-of-band shell. Keep useful project/task features without requiring every MCP client to understand a second transport or every devtools command to have its own Loki wrapper.
...
A03 implementation status: the finite no-network sandbox backend, privileged launcher role, one-shot jobs core/remote adapter, and unprivileged executor role are now implemented with strict Unix-RPC authority boundaries. Agent-facing executor requests contain only CWD/argv; job IDs and policy generations are supplied by trusted code. The cross-role fixture verifies that forged authority fields are rejected and launcher failures/timeouts do not become success. A03 is not considered fully closed until a real supported-host OCI integration proves project-controlled children cannot access control credentials, host namespaces/sockets or broader mounts. A05 still owns output streaming, explicit cancellation, durable job states, cgroup descendant cleanup/OOM handling and restart reconciliation; A07 remains the first MCP exposure.
