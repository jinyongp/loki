...
A03/A05 implementation status: project execution now has a typed one-shot executor API over a remote launcher adapter and a launcher-owned bounded asynchronous job registry. The launcher exposes finite `start`, `wait`, and `cancel` operations, applies a trusted run timeout, retains completed results for a bounded interval, and cancels owned runners during explicit cancellation or launcher shutdown. A `wait` RPC no longer owns workload lifetime; the remote adapter uses an independent cleanup context to send `cancel` after uncertain `start`/`wait` failures or caller cancellation. Cross-role Unix-socket regressions prove that Agent input cannot inject job IDs or policy digests and that executor-client cancellation reaches the launcher-owned runner before the trusted launcher timeout. This partial A05 lifecycle slice does not retire R7: readinessprobe still reproduces the detached-descendant baseline in the legacy `internal/process` path, while bounded output, durable job state and restart reconciliation, cgroup-owned descendant cleanup, OOM handling, and real supported-host OCI lifecycle acceptance remain.
...
The [package-structure follow-up](project-structure-plan.md) recorded the reviewed baseline graph with `go list -mod=readonly -json ./...`: 36 package directories, 85 direct in-module import edges and 33 immediate internal directories. At that baseline, `service` contained 23 production files and imported 24 internal packages; `cmd/loki` imported 21, and the graph included `auth -> browsernet -> portguard`, `devtools -> secret -> state`, and ordinary file/Git revision code importing `state.AtomicWrite`. S02 has since removed both misplaced shared-mechanism paths: destination validation/dial/proxy mechanics now live in `internal/platform/netguard`, and trusted-parent private atomic publication now lives in `internal/platform/safeio`, so Git checkpoints and workspace revisions no longer import the encrypted state package. Workload network grants, broader safeio extraction, and the remaining `devtools -> secret -> state` ownership problem are still future work. The baseline examples identify misplaced ownership rather than proving every high-fan-out package is wrong. The follow-up specifies responsibility groups, private feature internals, package target owners, role dependency checks and S01-S06 structural slices that accompany the functional A work units. Package enforcement starts early; it is not deferred until final cleanup.

The unused raw Docker forwarding session has been removed. The retained `internal/dockerproxy` code is inspection-only and is no longer classified as launcher authority. A03 now has a separate `internal/platform/sandbox` foundation, privileged `app/launcher` + `cmd/loki-launcher` boundary, `work/jobs` core with remote launcher adapter, and unprivileged `app/executor` + `cmd/loki-executor` role. The sandbox constructs a fixed non-root, read-only, no-network OCI security envelope and speaks only a bounded create/start/wait/kill/remove Docker API subset; executor requests contain only CWD/argv and cannot supply job IDs, policy digests, image/mount/security fields or launcher authority. These pieces are still internal and are not yet wired to MCP. Real supported-host OCI isolation and A05 lifecycle ownership remain required before treating this as a complete project-execution boundary. The signing proxy still exposes generic SSH signing requests, not a universal guarantee that every signature represents an approved Git commit.
