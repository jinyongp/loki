...
The unused raw Docker forwarding session has been removed. The retained `internal/dockerproxy` code is inspection-only and is no longer classified as launcher authority. A03 now has a separate `internal/platform/sandbox` foundation that constructs a fixed non-root, read-only, no-network OCI security envelope and speaks only a bounded create/start/wait/kill/remove Docker API subset; it is not yet exposed through a launcher RPC/service. The signing proxy still exposes generic SSH signing requests, not a universal guarantee that every signature represents an approved Git commit.
