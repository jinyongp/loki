...
A03 prototype status: project execution now has a typed one-shot jobs core, a remote launcher adapter that injects the trusted effective-policy digest, a dedicated unprivileged executor role, and a separate privileged launcher role backed by the finite sandbox API. Cross-role Unix-socket regressions prove that Agent input cannot inject job IDs or policy digests and that launcher rejection/timeouts remain errors. This does not retire R7: output streaming, explicit cancellation RPC, durable job state, restart reconciliation, cgroup-owned descendant cleanup, OOM handling and disconnect semantics remain A05 work, and readinessprobe still reproduces the detached-descendant baseline in the legacy process path.
